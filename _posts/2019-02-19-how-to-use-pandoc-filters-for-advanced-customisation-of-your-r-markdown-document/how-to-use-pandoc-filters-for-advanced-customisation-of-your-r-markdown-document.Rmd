...
However, as Haskell is not widely known, John MacFarlane also created the python package [`pandocfilters`](https://github.com/jgm/pandocfilters), and others made the more accessible alternative (still in python) [`panflute` package](https://github.com/sergiocorreia/panflute). Adaptations of the `pandocfilters` package has been developed for other languages, including [JavaScript](https://www.npmjs.com/package/pandoc-filter) and [R](https://cran.r-project.org/web/packages/pandocfilters/README.html)).

A potential problem with these options is that they create new dependencies, for example on a local Python installation, which means that additional stuff besides pandoc is necessary get them to work.
...
With that in mind (which will make sense of the syntax we use to solve the original problem in a minute), let's try building a couple of examples:
...
Our filter should find the YAML metadata in the abstract syntax tree (AST) and manipulate the author, before it lets pandoc convert it to a **.tex** file from which a PDF can be built.

To do this, create a function with the name of the element in the AST we want to manipulate, in this case `Meta`.
Then we manipulate the author and return a modified version:
...
If we save **toy_filter.lua** and try knitting **text.Rmd** to PDF again, this is the result:
...
First we take a look at the [documentation for pandoc lua filters](https://www.pandoc.org/lua-filters.html). We discover that italicised text passages are parsed by pandoc as `Emph` inline elements in the AST, and text passages in bold as `Strong` inline elements.
...
To avoid highlighting *all* span elements, I could also add a class 'correction' to the span, `<span class="correction">like this</span>`.
...
## Solving for PDF
For PDF output, I needed my lua filter to find `Span` elements, check if the class was "correction", and if so wrap the content in the `\mccorrect` command. This should do the trick:
